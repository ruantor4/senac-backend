...
public class RenameFile {
    public static void renomearArquivo() {
        try {
            ...
            String aleatoryName = String.format("nome-aleatorio-%s", newFileName);
            Path pathDestinationTemp = Paths.get(folderFullPath + "\\" + aleatoryName);
            if (
                fileFullPath.length() > 0 && 
                folderFullPath.length() > 0 &&
                fileName.length() > 0 &&
                newFileName.length() > 0) {
                Files.copy(pathOrigin, pathDestinationTemp, REPLACE_EXISTING);
                Files.delete(pathOrigin);
                Files.copy(pathDestinationTemp, pathDestination, REPLACE_EXISTING);
                Files.delete(pathDestinationTemp);
